 ...
 * From maxent.h
 */
public class ME_Model implements Serializable {
	...
	private final int MAX_LABEL_TYPES = 255;
	
	public ME_Model(String modelsPath) {
		_nheldout = 0;
		_early_stopping_n = 0;
		_ref_modelp = null;
	...
	private static class Sample {
		...
		private final boolean operator_less(final Sample x) {
			for (int i = 0; i < positive_features.length; i++) {
				if (i >= x.positive_features.length)
					return false;
	...
	class ME_Feature implements Serializable{
		private static final long serialVersionUID = 1L;
		public final int body; // unsigned
		public final int label;

		public ME_Feature(int l, int f, int maxLabelTypes) {
			assert (l >= 0 && l <= maxLabelTypes);
	...
	class ME_FeatureBag implements Serializable {
		private static final long serialVersionUID = 1L;
		// unsigned int, int
	...
	static class MiniStringBag implements Iterable<Entry<String, Integer>>, Serializable {
		int _size;
	...
	public int perform_GIS(int C) {
		C = 1;
		double[] pre_v = new double[_vl.length]; // jenia: same size as _vl's
													// because later it can set
													// _vl
	...
	public final int conditional_probability(final Sample s, double[] membp) {
		...
		for (int label = 0; label < _num_classes; label++) {
			membp[label] /= sum;
			if (membp[label] > membp[max_label])
				max_label = label;
		}
		assert (max_label >= 0);
		return max_label;
		...
		if (cutoff > 0) {
			for (Sample i : samples) {
				for (Integer j : i.positive_features) {
					//(f << 8) + l)
					increase(count, i.label << +  j);
				}
			}
		}

		for (Sample i : samples) {
			max_num_features = max(max_num_features,
					(i.positive_features.length));
			for (Integer j : i.positive_features) {
				final ME_Feature feature = new ME_Feature(i.label, j, MAX_LABEL_TYPES);
				if (cutoff > 0 && count.get(feature.body) <= cutoff)
					continue;
			}
		}
		count.clear();
	...
	public int train(final ArrayList<ME_Sample> vms, final int cutoff,
			final double sigma, final double widthfactor) {
		samples.clear();
		for (ME_Sample i : vms) {
	...
	int train(final int cutoff, final double sigma, final double widthfactor) {
		...
		for (Sample i : samples) {
			for (Integer j : i.positive_features) {
				for (Integer k : _feature2mef.get(j)) {
					if (_fb.Feature(k).label == i.label)
						plusEq(_vee, k, 1.0);
				}
			}
		}
	...
	void get_features(List<Tuple2<Tuple2<String, String>, Double>> fl) {
		...
		// for (MiniStringBag::map_type::final_iterator i =
		// _featurename_bag.begin(); i != _featurename_bag.end(); i++) {
		for (Entry<String, Integer> i : _featurename_bag) {
			for (int j = 0; j < _label_bag.Size(); j++) {
				...
				int id = _fb.Id(new ME_Feature(j, i.getValue(), MAX_LABEL_TYPES));
				if (id < 0)
					continue;
	...
	public boolean load_from_file(final String filename) {
		try {
			...
			/*
			 * jenia: the original algorithm read the file with a char buffer of
			 * size 1024. In main this is fixed as the maximum line size.
			 * Therefore the original algorithm was equivalent to read the file
			 * line by line.
			 */

			BufferedReader br = new BufferedReader(new InputStreamReader(this.getClass().getResourceAsStream(modelsPath + filename)));
			String line;
			while ((line = br.readLine()) != null) {
				...
				if (tokens.length < 3) {
					 ...
					 * TODO
					 * 
					 * All lines should contain exactly 3 tokens but this one is
					 * a bug:
					 * 
					 * "CONTAIN_UPPER 0.606178" (model.bidir.0)
					 * 
					 * The original doesn't treat this explicitly but when this
					 * happens classname is set to the empty String featurename
					 * to first token (typically second) and w to the second
					 * token (typically third)
					 */
				...
				String w = tokens[2];
				double lambda = Double.parseDouble(w);// jenia: was float
														// originally, but _vl
														// is vector of doubles

				int label = _label_bag.Put(classname);
				int feature = _featurename_bag.Put(featurename);
				_fb.Put(new ME_Feature(label, feature, MAX_LABEL_TYPES));
				_auxvl.add(lambda);
			}
	...
	void init_feature2mef() {
		...
		for (int i = 0; i < _featurename_bag.Size(); i++) {
			...
			for (int k = 0; k < _num_classes; k++) {
				int id = _fb.Id(new ME_Feature(k, i, MAX_LABEL_TYPES));
				if (id >= 0)
					vi.add(id);
			}
			_feature2mef.add(vi);
